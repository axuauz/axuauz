<div align="center">
  <img src="https://via.placeholder.com/150" alt="avatar" width="150" />
  
  <h1>Hi, I'm <strong>axuauz</strong></h1>
  <a href="https://github.com/DSHS-ACT">ACT(KE)</a> President
  <p>I'm Researcher<br>
  for System AI, Quantum Computing</p>

  <!-- Social badges -->
  <p>
    <a href="https://github.com/axuauz"><img src="https://img.shields.io/badge/GitHub-axuauz-black?style=flat-square&logo=github" alt="GitHub" /></a>
    <a href="mailto:nujxa@icloud.com"><img src="https://img.shields.io/badge/Email-your.email%40example.com-red?style=flat-square&logo=gmail" alt="Email" /></a>
  </p>
</div>

---

## Ability

- Antenna, Energy Process SIM
- Deep Learning Model Engineering

---

## Tech Stack 

<p align="center">
  <img src="https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/-C-00599C?style=flat-square&logo=c%2B%2B&logoColor=white" />
  <img src="https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white" />
  <img src="https://img.shields.io/badge/-TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white" />
  <img src="https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/-Overleaf-000000?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white" />
</p>

---

## Section (UI 레이아웃 예시)

| 📄 논문 &amp; 보고서 | 🔗 프로젝트 링크 | 🧪 시뮬레이션 결과 |
|-------------------|------------------|---------------------|
| "논문 제목" (2025)<br>간단 설명 | [프로젝트 A](https://github.com/axuauz/project-a)<br>시뮬레이션 엔진 | 결과 설명 + 이미지/그래프 링크 |
| "논문 제목 2" (2024)<br>간단 설명 | [프로젝트 B](https://github.com/axuauz/project-b)<br>데이터 모델링 | 결과 설명 + 시각 자료 |

> 이 표는 예시입니다. 필요에 따라 컬럼을 추가하거나 변경하세요.
